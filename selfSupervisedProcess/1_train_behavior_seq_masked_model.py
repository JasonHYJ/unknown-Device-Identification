# train_behavior_seq_masked_model.py

"""
功能说明：
该脚本用于训练一个自监督模型（Transformer Encoder），用于行为流量的序列特征建模。
训练目标为掩码预测任务（Masked Sequence Modeling, MSM），输入为 [pkt_len, iat, direction] 的多通道序列，
通过 MLP 投影、位置编码、Transformer 编码器进行建模，最终训练后保存模型权重，可用于后续嵌入向量提取。
输入数据来源于 uk_train.csv 中 is_behavior=1 的训练样本（即行为样本）。
输出模型保存为：behavior_seq_model.pt
新增：对输入样本进行归一化处理，确保与嵌入阶段一致。
"""

import os
os.environ["CUDA_VISIBLE_DEVICES"] = "2"  # 可改为 "0" 或 ""，根据GPU使用情况
...
save_model_path = Path("/home/hyj/unknownDeviceIdentification/selfSupervisedProcess/3rd/cicIoT2022_behavior_seq_model.pt")

log_dir = Path("/home/hyj/unknownDeviceIdentification/selfSupervisedProcess/3rd/train_logs")
log_dir.mkdir(parents=True, exist_ok=True)
log_file = log_dir / "cicIoT2022_behavior_seq_train.log"
...
# ------------------------- 数据集定义 -------------------------
class BehaviorMaskedDataset(Dataset):
...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, max_len=256):  # 行为序列最大长度为 256
        super().__init__()
...
class MaskedSeqModel(nn.Module):
    def __init__(self, input_dim=3, embed_dim=128, trans_dim=128, nhead=4, nlayers=2):
        ...
        self.pos_encoder = PositionalEncoding(embed_dim, max_len=256)
...
# ------------------------- 训练主流程 -------------------------
def train():
    dataset = BehaviorMaskedDataset(csv_path)
    dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=True, drop_last=True)
    model = MaskedSeqModel().to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=lr)
    criterion = nn.MSELoss()

    log(f"📚 开始训练行为模型，总样本数: {len(dataset)}, 批大小: {batch_size}, 总轮数: {num_epochs}")
