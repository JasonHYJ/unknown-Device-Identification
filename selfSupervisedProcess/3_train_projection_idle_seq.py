# train_projection_idle_seq.py
"""
📌 功能说明：
本脚本对 UK 数据集中“闲时流量的序列嵌入向量（128维）”进行对比学习训练，
训练一个投影模型（MLP），将其映射到更具判别力的64维嵌入空间，并保存为 .pt 文件。

特点：
- 输入数据：每台设备的闲时流量序列嵌入（128维 .npy），路径结构保持不变。
...
SAVE_MODEL_PATH = "/home/hyj/unknownDeviceIdentification/selfSupervisedProcess/3rd/cicIoT2022_projection_idle_seq.pt"
EMBEDDING_DIM = 128
PROJECTED_DIM = 64
BATCH_SIZE = 128
EPOCHS = 20
...
# Step 1: 数据集定义
# ==========================
class TripletIdleSeqDataset(Dataset):
    def __init__(self, root):
        ...
        print(f"✅ 加载闲时序列嵌入样本: {len(self.samples)}，设备数: {len(self.device2files)}")
...
# ==========================
def train():
    dataset = TripletIdleSeqDataset(INPUT_ROOT)
