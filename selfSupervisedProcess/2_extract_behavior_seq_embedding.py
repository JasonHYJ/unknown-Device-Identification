"""
功能说明：
该脚本用于对 uk 数据集中划分为 train 的行为序列特征样本，
使用已训练好的 Transformer Encoder 模型提取嵌入向量。
每个样本输入为 [pkt_len, iat, direction] 特征的行为序列矩阵（最多 256 个数据包），
输出为一个 128 维嵌入向量 z_seq，保存为 _seq_embed.npy。
"""
...
class MaskedSeqModel(nn.Module):
    ...
    def forward(self, x):  # 仅 forward encoder
        x = self.mlp(x)
...
def main():
    ...
    model_path = Path("/home/hyj/unknownDeviceIdentification/selfSupervisedProcess/3rd/cicIoT2022_behavior_seq_model.pt")
    ...
    dataset = BehaviorSequenceDataset(csv_path)
    dataloader = DataLoader(dataset, batch_size=1, shuffle=False)

    print(f"📦 共有行为训练样本数: {len(dataset)}")

    with torch.no_grad():
        for feature_matrix, npz_path in tqdm(dataloader, desc="🧠 提取行为嵌入"):
            feature_matrix = feature_matrix.to(device)             # [1, 256, 3]
            z = model(feature_matrix)                              # [1, 256, d]
            z_seq = z.mean(dim=1).squeeze(0)                       # [128]
            save_embedding(z_seq, npz_path[0], input_root, output_root)

    print("✅ 所有行为嵌入向量提取完成:", output_root)
