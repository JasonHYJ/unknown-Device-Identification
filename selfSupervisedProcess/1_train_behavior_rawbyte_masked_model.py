...
save_model_path = Path("/home/hyj/unknownDeviceIdentification/selfSupervisedProcess/3rd/cicIoT2022_behavior_rawbyte_model.pt") # 这里要加个数据集前缀
log_file = Path("/home/hyj/unknownDeviceIdentification/selfSupervisedProcess/3rd/train_logs/cicIoT2022_behavior_rawbyte_train.log")    # 这里要加个数据集前缀
...
# ------------------------ 数据集类（行为流量） ------------------------
class BehaviorRawByteDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return torch.tensor(matrix, dtype=torch.long)  # [256, 128]
...
class ByteModel(nn.Module):
    def __init__(self, vocab_size=257, embed_dim=64, cnn_dim=128, nhead=4, nlayers=2):
        ...
        self.decoder = nn.Linear(cnn_dim, vocab_size - 1)  # 预测 0~255 字节值
...
# ------------------------ 训练主函数 ------------------------
def train():
    dataset = BehaviorRawByteDataset(csv_path)
