"""
功能说明：
该脚本使用训练好的 ByteModel，提取每个闲时样本的全局嵌入向量 z_raw ∈ ℝ^128，
方法为：输入原始字节特征矩阵（[128, 128]），经过 ByteEmbedding → CNN → Transformer 后，
对输出进行 mean pooling 得到样本级向量。
结果保存为 xxx_raw_embed.npy，与原始样本路径结构一致。
"""

# extract_idle_rawbyte_embedding.py
...
class ByteModel(nn.Module):
    ...
    def forward(self, x):  # [B, N, L]
        ...
        return x.view(B, N, -1).mean(dim=1)  # [B, 128]
...
def main():
    ...
    model_path = "/home/hyj/unknownDeviceIdentification/selfSupervisedProcess/3rd/cicIoT2022_idle_rawbyte_model.pt"
