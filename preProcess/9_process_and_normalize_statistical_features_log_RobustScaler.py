...
- 对每个 CSV 文件的前 30 维统计特征进行归一化处理（先使用log变换处理极端值，再使用 RobustScaler），并处理 NaN 值。
...
def process_dataset(csv_path, set_type, scaler, train_medians, train_min, train_max, output_base):
    ...
    log_file = os.path.join(output_base, f"cicIoT2022_{set_type}_outliers.log")     # 这里要记得调整
    stats_file = os.path.join(output_base, f"cicIoT2022_{set_type}_outlier_stats.txt")  # 这里也要记得调整
    ...
    for idx, row in df.iterrows():
        ...
        try:
            ...
            if features_scaled.size > 0:
                features_normalized = (features_scaled - np.min(features_scaled)) / \
                                     (np.max(features_scaled) - np.min(features_scaled) + 1e-10)
            else:
            ...
            relative_path = file_path.split('/7_cleaned_features/7_cleaned_statistical_feature/cicIoT2022/')[-1]    # 这里要记得调整
            output_path = os.path.join(output_base, set_type, '', 'cicIoT2022', relative_path)  # 这里一定要记得调整！！！！
...
def main():
    ...
    """
    # 输入路径
    csv_path_train = "/home/hyj/unknownDeviceIdentification/dataset/8_split_sample_info/cicIoT2022/3_cicIoT2022_train.csv"
    csv_path_test = "/home/hyj/unknownDeviceIdentification/dataset/8_split_sample_info/cicIoT2022/3_cicIoT2022_test.csv"
    csv_path_unknown = "/home/hyj/unknownDeviceIdentification/dataset/8_split_sample_info/cicIoT2022/3_cicIoT2022_unknown.csv"
