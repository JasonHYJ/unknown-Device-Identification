...
# 📆 多任务IoT设备识别经典培训脚本（CICIOT2022专用，采样测试集）
...
from sklearn.model_selection import train_test_split  # 添加分层采样依赖
import pandas as pd

"""
(当前主要使用的用于训练模型的脚本，包含验证集，每次epoch的各项标签的效果)
cicIoT2022数据集由于测试样本过多，因此该数据集的代码添加了一项随机选择部分test样本进行实验的功能，
相应的，在evaluate代码中选择的test项的路径也将导向本代码中得到的sample_test的路径
脚本功能说明：
...
型号；
使用多任务加权损失进行联合训练；
最终将训练好的模型保存到：/home/hyj/unknownDeviceIdentification/dataset/11_multitask_training/cicIoT2022/optimized_multitask_model.pt
...
os.environ["CUDA_VISIBLE_DEVICES"] = "3"  # 设置使用GPU 0

# --------------------------- 参数配置 ---------------------------
train_csv_path = "/home/hyj/unknownDeviceIdentification/dataset/11_multitask_training/cicIoT2022/3_cicIoT2022_train.csv"  # 训练集CSV路径
test_csv_path = "/home/hyj/unknownDeviceIdentification/dataset/11_multitask_training/cicIoT2022/3_cicIoT2022_test.csv"  # 验证集CSV路径
...
label_dict_dir = "/home/hyj/unknownDeviceIdentification/dataset/11_multitask_training/cicIoT2022"  # 标签字典目录

output_model_path = "/home/hyj/unknownDeviceIdentification/dataset/11_multitask_training/cicIoT2022/3_optimized_multitask_model.pt"  # 模型保存路径
...
sample_size = 30000  # 新增：测试集采样规模，适用于所有数据集

# --------------------------- 采样测试集 ---------------------------
"""
` 添加了 sample_test_set 函数，使用 train_test_split 进行分层采样，按 type_label 和 is_behavior 确保分布一致。
` 函数检查测试集规模是否小于 sample_size，若小于则使用原始测试集。
` 输出采样前后的 type_label 和 is_behavior 分布，便于验证代表性。
"""
def sample_test_set(input_csv_path, output_csv_path, sample_size):
    """对测试集进行分层采样，保持类别和is_behavior分布"""
    test_df = pd.read_csv(input_csv_path)
    if len(test_df) <= sample_size:
        print(f"Test set size ({len(test_df)}) is smaller than or equal to sample_size ({sample_size}), using original test set.")
        return input_csv_path
    sampled_test_df, _ = train_test_split(
        test_df,
        train_size=sample_size,
        stratify=test_df[['type_label', 'is_behavior']],
        random_state=42
    )
    sampled_test_df.to_csv(output_csv_path, index=False)
    print(f"Sampled test set saved to: {output_csv_path}")
    print(f"Sampled test samples: {len(sampled_test_df)}")
    print(f"Original type_label distribution: {test_df['type_label'].value_counts().to_dict()}")
    print(f"Sampled type_label distribution: {sampled_test_df['type_label'].value_counts().to_dict()}")
    print(f"Original is_behavior distribution: {test_df['is_behavior'].value_counts().to_dict()}")
    print(f"Sampled is_behavior distribution: {sampled_test_df['is_behavior'].value_counts().to_dict()}")
...
def train():
    # 采样测试集
    sampled_test_csv_path = "/home/hyj/unknownDeviceIdentification/dataset/11_multitask_training/cicIoT2022/3_sampled_cicIoT2022_test.csv"
    test_csv_path_used = sample_test_set(test_csv_path, sampled_test_csv_path, sample_size)

    # 加载训练和验证数据集
    train_dataset = MultiModalIoTDataset(train_csv_path, root_stat, root_seq, root_raw, label_dict_dir)  # 训练数据集
    test_dataset = MultiModalIoTDataset(test_csv_path_used, root_stat, root_seq, root_raw, label_dict_dir)  # 验证数据集
