...
# 📆 多任务IoT设备识别评估脚本（CICIOT2022专用，采样测试集）

# 功能说明：
# 由于2_optimized_train_multitask_classifier_cicIoT2022.py中随机采样了一部分的test数据，所以这里的test数据的输入要保持一致，应该使用sampled_cicIoT2022_test.csv
# 所以把这个数据集单独独立出来了一个样本.

# 本脚本用于评估多任务IoT设备识别模型的性能，支持对已知设备（测试集）和未知设备（未知集）进行分类。
# - 加载预训练的多任务分类模型（optimized_multitask_model.pt），处理统计特征、序列嵌入和原始字节嵌入。
# - 对测试集（sampled_cicIoT2022_test.csv）进行采样并评估类型（type）、品牌（brand）和型号（device）的分类准确率和F1分数。
# - 对未知集（cicIoT2022_unknown.csv）评估模型对未见设备的泛化能力，计算未知型号识别率（device_unknown_rate）。
...
log_dir = "/home/hyj/unknownDeviceIdentification/dataset/11_multitask_training/cicIoT2022/3_eval_results"
...
# --------------------------- 参数配置 ---------------------------
# 数据和模型路径
# 3_evaluate_multitask_classifier.py 应使用相同的 sampled_cicIoT2022_test.csv 作为测试集，确保训练和评估的验证集一致。
test_csv_path = "/home/hyj/unknownDeviceIdentification/dataset/11_multitask_training/cicIoT2022/3_sampled_cicIoT2022_test.csv"  # 测试集CSV
unknown_csv_path = "/home/hyj/unknownDeviceIdentification/dataset/11_multitask_training/cicIoT2022/3_cicIoT2022_unknown.csv"  # 测试集CSV
...
model_path = "/home/hyj/unknownDeviceIdentification/dataset/11_multitask_training/cicIoT2022/3_optimized_multitask_model.pt"  # 预训练模型路径

output_dir = "/home/hyj/unknownDeviceIdentification/dataset/11_multitask_training/cicIoT2022/3_eval_results"  # 输出目录
batch_size = 64  # 批次大小
confidence_threshold = 0.5  # 类型和品牌预测的置信度阈值
device_threshold = 0.3  # 型号预测的置信度阈值（低于此值预测为未知）
            ...
            try:
                ...
                for i in range(len(type_pred)):
                    ...
                    results.append({
                        ...
                        'pred_device': idx2device[device_pred[i]],  # 品牌最大概率
...
def main():
    ...
    try:
        ...
        if missing_cols:
            logger.error(f"Missing columns in 3_sampled_cicIoT2022_test.csv: {missing_cols}")
            sys.exit(1)
        ...
        logger.error(f"Failed to load 3_sampled_cicIoT2022_test.csv: {e}")
        sys.exit(1)
    ...
    try:
        ...
        if missing_cols:
            logger.error(f"Missing columns in 3_cicIoT2022_unknown.csv: {missing_cols}")
            sys.exit(1)
        ...
        logger.error(f"Failed to load 3_cicIoT2022_unknown.csv: {e}")
        sys.exit(1)
